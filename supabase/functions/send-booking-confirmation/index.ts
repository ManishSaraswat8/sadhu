...
serve(async (req) => {
  ...
  try {
    ...
    const { data: session, error: sessionError } = await supabaseClient
      ...
      .select(`
        *,
        practitioner:practitioners(id, name, bio),
    ...
    // Get client email from auth.users if not in profile
    ...
    const sessionUrl = `${siteUrl}/sessions/${session_id}`;

    // Get Postmark template configuration
    const templateId = Deno.env.get("POSTMARK_BOOKING_CONFIRMATION_TEMPLATE_ID");
    const templateAlias = Deno.env.get("POSTMARK_BOOKING_CONFIRMATION_TEMPLATE_ALIAS");
    ...
    if (templateId || templateAlias) {
      ...
      emailResult = await sendPostmarkEmail({
        to: clientEmail,
        subject: `Booking Confirmed: ${sessionTypeText} with ${practitionerName}`,
        htmlBody: `
